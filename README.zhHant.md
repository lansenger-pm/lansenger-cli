...
lansenger config set identity_type personal-bot   # 可選：憑證身份類型 — personal-bot（個人機器人）/ org-app（組織應用）/ org-bot（組織機器人）
```
...
| `message` | 傳送與管理訊息 | `send-text`, `send-markdown`, `send-file`, `send-image-url`, `send-link-card`, `send-app-articles`, `send-app-card`, `send-oacard`, `send-bot-message`, `send-group-message`, `send-account-message`, `send-user-message`, `update-dynamic-card`, `revoke`, `query-groups` |
...
```bash
# 查詢官方帳號（code 列即发送所需的 accountCode）
lansenger notice accounts --org-id org001

# 發送文字通知，按手機號投放（接收/抄送各最多 10 个）
...
# 帶 --as / --user-token 時，--create-mobile / --create-user-id 可省略
...
| `calendar *` | N | **Y** | **Y** | 有 userToken = 使用者身份；無 = 機器人身份 |
| `todo *` | N | **Y** | **Y** | 僅組織級應用 |
...
> **個人機器人**只能收發訊息和上傳/下載檔案。不能存取通訊錄、行事曆或 OAuth2。
>
> **組織應用 vs 組織應用 + 機器人**：使用相同的 appID/appSecret。唯一區別在於訊息通道——只有機器人可以傳送機器人私聊和群訊息（因為只有機器人可以加入群組）。所有其他 API（通訊錄、行事曆、待辦、會話、OAuth2、串流訊息）兩者完全一致。目前僅自建應用支援機器人能力。
...
| 上傳媒體 | 上傳媒體檔案權限 | `lansenger-media`（upload、upload-app） |
| 工作台模板讀取 | 工作台模板讀取權限 | — |
| 工作台模板寫入 | 工作台模板寫入權限 | — |
